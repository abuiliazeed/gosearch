...
type Config struct {
	...
	// CORS allowed origins (comma-separated, use "*" for wildcard)
	CORSAllowedOrigins string `mapstructure:"cors-allowed-origins"`
	// CORS allowed methods (comma-separated: GET, POST, PUT, DELETE, OPTIONS)
...
func setDefaults() {
	...
	viper.SetDefault("cors-allowed-methods", "GET, POST, PUT, DELETE, OPTIONS")
	viper.SetDefault("cors-allowed-headers", "Content-Type, Authorization")
}
