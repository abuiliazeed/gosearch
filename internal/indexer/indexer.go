...
//
// Returns an error if the document cannot be indexed or context is canceled.
func (i *Indexer) IndexDocument(ctx context.Context, doc *storage.Document) error {
	...
	indexContent := MarkdownToText(doc.ContentMarkdown)
	if doc.URL != "" {
		// Include URL tokens so domain-style queries like "sourcebeauty" can match.
		if indexContent == "" {
			indexContent = doc.URL
		} else {
			indexContent += " " + doc.URL
		}
	}
...
func (i *Indexer) Save(ctx context.Context) error {
	...
	savedTerms := 0
	postingsBatch := make([]*storage.PersistedPostingsList, 0, len(indexData.terms))
	for term, plist := range indexData.terms {
		...
		postingsBatch = append(postingsBatch, persistedList)

		// Batch save every 1000 terms to avoid massive transactions
		if len(postingsBatch) >= 1000 {
			if err := i.store.SavePostingsBatch(postingsBatch); err != nil {
				return fmt.Errorf("failed to save postings batch: %w", err)
			}
			savedTerms += len(postingsBatch)
			postingsBatch = postingsBatch[:0] // Clear slice
		}
	}

	// Save remaining postings
	if len(postingsBatch) > 0 {
		if err := i.store.SavePostingsBatch(postingsBatch); err != nil {
			return fmt.Errorf("failed to save remaining postings batch: %w", err)
		}
		savedTerms += len(postingsBatch)
	}

	// Save each document's metadata
	docInfoBatch := make([]*storage.PersistedDocInfo, 0, len(indexData.docs))
	savedDocs := 0
	for _, docInfo := range indexData.docs {
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
		...
		docInfoBatch = append(docInfoBatch, persistedDocInfo)

		// Batch save every 1000 docs
		if len(docInfoBatch) >= 1000 {
			if err := i.store.SaveDocInfoBatch(docInfoBatch); err != nil {
				return fmt.Errorf("failed to save doc info batch: %w", err)
			}
			savedDocs += len(docInfoBatch)
			docInfoBatch = docInfoBatch[:0]
		}
	}

	// Save remaining doc info
	if len(docInfoBatch) > 0 {
		if err := i.store.SaveDocInfoBatch(docInfoBatch); err != nil {
			return fmt.Errorf("failed to save remaining doc info batch: %w", err)
		}
		savedDocs += len(docInfoBatch)
	}
...
func (i *Indexer) Rebuild(_ context.Context, _ *storage.DocumentStore) error {
