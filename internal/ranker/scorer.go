...
func (s *Scorer) computeCombinedScore(docID string, docInfo *indexer.DocInfo, tfidfScore, prScore float64, queryTerms []string) float64 {
...
func (s *Scorer) computeTitleBoost(docInfo *indexer.DocInfo, queryTerms []string) float64 {
	...
	for _, term := range queryTerms {
		if contains(titleLower, toLower(term)) {
			matchCount++
		}
	}
