...
// Strategy represents the crawling strategy for URL ordering.
type Strategy string

const (
	// StrategyBFS uses breadth-first search (default recommended).
	StrategyBFS Strategy = "bfs"
	// StrategyBestFirst prioritizes URLs by potential relevance.
...
func ParseStrategy(s string) Strategy {
	...
	case "bfs", "BFS", "breadth-first":
		return StrategyBFS
	case "best-first":
		return StrategyBestFirst
	default:
		return StrategyBFS // Default to BFS
	}
}
...
type Config struct {
	...
	Strategy          Strategy      // Crawling strategy (bfs, best-first)
	MaxQueueSize      int           // Maximum number of URLs to queue (0 = unlimited)
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		MaxRetries:        3,
		Strategy:          StrategyBFS, // Default to BFS
	}
}
