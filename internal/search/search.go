...
func NewSearcher(idx *indexer.Index, scorer *ranker.Scorer, cache *storage.CacheStore, config *Config) *Searcher {
	if config == nil {
...
func (s *Searcher) Search(ctx context.Context, query string) (*SearchResponse, error) {
	...
	// Build response
	results := make([]*Result, 0, len(rankedResults))
...
func (s *Searcher) UpdateConfig(config *Config) {
	s.config = config
